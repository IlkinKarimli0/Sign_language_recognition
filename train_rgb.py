...
modelo.load_weights('/home/nigar.alishzada/SLR/keras-kinetics-i3d/weights_rgb/weights-7.h5')
modelo.summary()

#Creating data object for streaming 
data = DataLoader('WL_AzSL/videos_by_names')
# data streaming
train_ds,test_ds = data.stream_line(n_frames = 20,output_size=(224,224),frame_step = 1)
...
if not os.path.exists('weights_rgb'):
    os.makedirs('weights_rgb')
...
for epoch in range(epochs):
    ...
    if len(test_loss_results) > 1:
        if test_loss_results[-1] < lowest:
            modelo.save_weights(f"weights_rgb/weights-{epoch + 1}-{val_loss_metric.result():.2f}.h5")
...
fig.savefig('training_reports/train_report_rgb'+train_num+'.png')
...
fig.savefig('training_reports/test_report_rgb_'+train_num+'.png')
